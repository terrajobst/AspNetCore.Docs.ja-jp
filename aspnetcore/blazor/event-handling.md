...
Razor コンポーネントでは、イベント処理機能が提供されます。 `on{EVENT}` という名前の HTML 要素属性 (`onclick`、`onsubmit` など) でデリゲート型の値がある場合、Razor コンポーネントでは、属性の値がイベント ハンドラーとして扱われます。 属性の名前は常に [`@on{EVENT}`](xref:mvc/views/razor#onevent) という形式になります。

...
ブラウザーのセキュリティでは、web ページが web ページを提供したものとは異なるドメインに要求を作成しないようにします。 この制限は、*同じオリジンポリシー*と呼ばれます。 同じ配信元ポリシーでは、悪意のあるサイトが別のサイトから機密データを読み取れないようにします。 場合によっては、他のサイトがアプリに対してクロスオリジン要求を行うことを許可する必要があります。 詳細については、「 [MOZILLA CORS](https://developer.mozilla.org/en-US/docs/Web/HTTP/CORS)」を参照してください。
...
* `https://example.net` &ndash; つの異なるドメイン
* `https://www.example.com/foo.html` &ndash; つの異なるサブドメイン
* `http://example.com/foo.html` &ndash; つの異なるスキーム
* `https://example.com:9000/foo.html` &ndash; つの異なるポート
...
* ポリシー名を "\_Myallow" オリジン "に設定します。 ポリシー名は任意です。
* @No__t-0 拡張メソッドを呼び出します。これにより、CORS が有効になります。
* [ラムダ式](/dotnet/csharp/programming-guide/statements-expressions-operators/lambda-expressions)を使用して <xref:Microsoft.Extensions.DependencyInjection.CorsServiceCollectionExtensions.AddCors*> を呼び出します。 ラムダは @no__t 0 のオブジェクトを受け取ります。 @No__t-1 などの[構成オプション](#cors-policy-options)については、この記事の後半で説明します。

@No__t 0 のメソッド呼び出しは、アプリのサービスコンテナーに CORS サービスを追加します。

[!code-csharp[](cors/sample/Cors/WebAPI/Startup.cs?name=snippet2)]

詳細については、このドキュメントの「 [CORS ポリシーオプション](#cpo)」を参照してください。

@No__t-0 メソッドでは、次のコードに示すようにメソッドを連結できます。
...
```
注: `UseMvc` の前に `UseCors` を呼び出す必要があります。
...
エンドポイントルーティングでは、@no__t 0 の拡張メソッドセットを使用して、エンドポイントごとに CORS を有効にすることができます。
...
[@No__t-1 Enablecor @ no__t-2](xref:Microsoft.AspNetCore.Cors.EnableCorsAttribute)属性は、CORS をグローバルに適用するための代替手段を提供します。 @No__t-0 属性を指定すると、すべてのエンドポイントではなく、選択したエンドポイントに対して CORS が有効になります。

@No__t-0 を使用して既定のポリシーを指定し、`[EnableCors("{Policy String}")]` を指定してポリシーを指定します。

@No__t-0 属性は、次のように適用できます。

* Razor ページ `PageModel`
* コントローラー
* コントローラーアクションメソッド

@No__t-0 属性を使用して、コントローラー/ページモデル/アクションに異なるポリシーを適用できます。 @No__t-0 属性がコントローラー/ページモデル/アクションメソッドに適用され、CORS がミドルウェアで有効になっている場合、両方のポリシーが適用されます。 ポリシーを組み合わせることはお勧めしません。 同じアプリ内ではなく、`[EnableCors]` 属性またはミドルウェアを使用します。

次のコードは、各メソッドに異なるポリシーを適用します。

[!code-csharp[](cors/sample/Cors/WebAPI/Controllers/WidgetController.cs?name=snippet&highlight=6,14)]

次のコードでは、CORS の既定ポリシーと `"AnotherPolicy"` という名前のポリシーを作成します。

[!code-csharp[](cors/sample/Cors/WebAPI/StartupMultiPolicy.cs?name=snippet&highlight=12-28)]

### <a name="disable-cors"></a>CORS を無効にする

[@No__t-1 Disablecor @ no__t-2](xref:Microsoft.AspNetCore.Cors.DisableCorsAttribute)属性は、コントローラー/ページモデル/アクションの CORS を無効にします。
...
<xref:Microsoft.AspNetCore.Cors.Infrastructure.CorsOptions.AddPolicy*> が `Startup.ConfigureServices` で呼び出されます。 いくつかのオプションについては、まず「 [CORS のしくみ](#how-cors)」セクションを参照してください。

## <a name="set-the-allowed-origins"></a>許可されるオリジンの設定

<xref:Microsoft.AspNetCore.Cors.Infrastructure.CorsPolicyBuilder.AllowAnyOrigin*> &ndash; を使用すると、すべてのオリジンから任意のスキーム (`http` または `https`) の CORS 要求を実行できます。 *web サイト*はアプリに対してクロスオリジン要求を行うことができるため、`AllowAnyOrigin` は安全ではありません。

::: moniker range=">= aspnetcore-2.2"

> [!NOTE]
> @No__t-0 および `AllowCredentials` を指定することは安全ではない構成であり、クロスサイト要求の偽造が発生する可能性があります。 アプリが両方の方法で構成されている場合、CORS サービスは無効な CORS 応答を返します。
...
> [!NOTE]
> @No__t-0 および `AllowCredentials` を指定することは安全ではない構成であり、クロスサイト要求の偽造が発生する可能性があります。 セキュリティで保護されたアプリの場合は、クライアントがサーバーリソースへのアクセスを承認する必要がある場合は、オリジンの正確な一覧を指定します。
...
<xref:Microsoft.AspNetCore.Cors.Infrastructure.CorsPolicyBuilder.SetIsOriginAllowedToAllowWildcardSubdomains*> &ndash; に設定すると、ポリシーの <xref:Microsoft.AspNetCore.Cors.Infrastructure.CorsPolicy.IsOriginAllowed*> プロパティは、オリジンが許可されているかどうかを評価するときに、構成されたワイルドカードドメインとオリジンを一致させることができる関数になります。
...
CORS 要求で特定のヘッダーを送信できるようにするには、 *author 要求ヘッダー*と呼ばれ、<xref:Microsoft.AspNetCore.Cors.Infrastructure.CorsPolicyBuilder.WithHeaders*> を呼び出し、許可されているヘッダーを指定します。
...
@No__t-0 によって指定された特定のヘッダーに対して CORS ミドルウェアポリシーが一致するのは、`Access-Control-Request-Headers` で送信されたヘッダーが `WithHeaders` で示されているヘッダーと完全に一致する場合のみです。
...
@No__t-0 ([Headernames](xref:Microsoft.Net.Http.Headers.HeaderNames.ContentLanguage)) が `WithHeaders` に一覧表示されていないため、CORS ミドルウェアは次の要求ヘッダーを使用してプレフライト要求を拒否します。
...
CORS ミドルウェアでは、CorsPolicy で構成されている値に関係なく、@no__t 0 の4つのヘッダーを常に送信できます。 このヘッダーの一覧には次のものが含まれます。
...
@No__t-0 は常にホワイトリストに登録されているため、CORS ミドルウェアは次の要求ヘッダーを使用してプレフライト要求に正常に応答します。
...
HTTP 応答には `Access-Control-Allow-Credentials` ヘッダーが含まれています。これにより、サーバーがクロスオリジン要求の資格情報を許可していることがブラウザーに通知されます。

ブラウザーが資格情報を送信しても、応答に有効な @no__t 0 ヘッダーが含まれていない場合、ブラウザーはアプリへの応答を公開せず、クロスオリジン要求は失敗します。
...
CORS の仕様では、`Access-Control-Allow-Credentials` ヘッダーが存在する場合は、`"*"` (すべてのオリジン) に設定することもできます。
...
* アプリでは、`Accept`、`Accept-Language`、`Content-Language`、`Content-Type`、また `Last-Event-ID` 以外の要求ヘッダーが設定されていません。
* @No__t-0 ヘッダーが設定されている場合は、次のいずれかの値になります。
...
* `Access-Control-Request-Headers`: アプリが実際の要求に対して設定する要求ヘッダーの一覧。 前述のように、これには、`User-Agent` など、ブラウザーが設定するヘッダーは含まれません。

CORS のプレフライト要求には、@no__t 0 のヘッダーが含まれる場合があります。これは、実際の要求と共に送信されるヘッダーをサーバーに示します。
...
ブラウザーは、`Access-Control-Request-Headers` に設定する方法と完全には一貫していません。 ヘッダーを `"*"` 以外 (または <xref:Microsoft.AspNetCore.Cors.Infrastructure.CorsPolicy.AllowAnyHeader*>) 以外の値に設定する場合は、少なくとも `Accept`、`Content-Type`、および @no__t 4 と、サポートするカスタムヘッダーを含める必要があります。
...
応答には、許可されているメソッドと、必要に応じて、許可されているヘッダーの一覧を示す `Access-Control-Allow-Headers` ヘッダーを一覧表示する `Access-Control-Allow-Methods` ヘッダーが含まれます。 プレフライト要求が成功した場合、ブラウザーは実際の要求を送信します。
...
@No__t-0 ヘッダーは、プレフライト要求への応答をキャッシュできる期間を指定します。 このヘッダーを設定するには、<xref:Microsoft.AspNetCore.Cors.Infrastructure.CorsPolicyBuilder.SetPreflightMaxAge*> を呼び出します。
...
次に、クロスオリジン要求の例を示します。 @No__t-0 ヘッダーは、要求を行っているサイトのドメインを提供します。 @No__t-0 ヘッダーは必須であり、ホストとは異なる必要があります。
...
サーバーが要求を許可している場合は、応答に `Access-Control-Allow-Origin` ヘッダーが設定されます。 このヘッダーの値は、要求の `Origin` ヘッダーと一致しているか、ワイルドカード値 `"*"` です。つまり、すべての配信元が許可されます。
...
1. このドキュメントのいずれかの方法を使用して CORS を有効にします。 (例:
  ...
  > [!WARNING]
  > `WithOrigins("https://localhost:<port>");` は、[ダウンロードサンプルコード](https://github.com/aspnet/AspNetCore.Docs/tree/live/aspnetcore/security/cors/sample/Cors)に似たサンプルアプリのテストにのみ使用してください。
...
1. 前のコードでは、`url: 'https://<web app>.azurewebsites.net/api/values/1',` を、デプロイされたアプリの URL に置き換えます。
...
1. @No__t-0 から localhost の発信元を削除し、アプリをデプロイします。 または、別のポートを使用してクライアントアプリを実行します。 たとえば、Visual Studio からを実行します。
1. クライアントアプリでテストします。 CORS エラーはエラーを返しますが、エラーメッセージは JavaScript では使用できません。 F12 ツールの [コンソール] タブを使用して、エラーを確認します。 ブラウザーによっては、次のようなエラー (F12 ツールコンソール) が表示されます。
   ...
   * Microsoft Edge を使用する:
     ...
     **SEC7120: [CORS] オリジン `https://localhost:44375` で、`https://webapi.azurewebsites.net/api/values/1` のクロスオリジンリソースのアクセス制御-@no__t 応答ヘッダーに-2 が見つかりませんでした**

   * Chrome を使用する:

     **オリジン @no__t から `https://webapi.azurewebsites.net/api/values/1` の XMLHttpRequest へのアクセスが CORS ポリシーによってブロックされています。要求されたリソースに ' アクセス制御許可-オリジン ' ヘッダーが存在しません。**
     
CORS が有効なエンドポイントは、 [Fiddler](https://www.telerik.com/fiddler)や[Postman](https://www.getpostman.com/)などのツールを使用してテストできます。 ツールを使用する場合は、`Origin` ヘッダーによって指定された要求の配信元が、要求を受信しているホストと異なる必要があります。 要求が `Origin` ヘッダーの値に基づいて*クロスオリジン*でない場合は、次のようになります。

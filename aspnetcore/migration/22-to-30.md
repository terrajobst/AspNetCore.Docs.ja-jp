...
* 設定、[ターゲット フレームワーク モニカー (TFM)](/dotnet/standard/frameworks)に`netcoreapp3.0`:
...
* Update`Startup.ConfigureServices`を呼び出す`AddNewtonsoftJson`します。
  ...
  `AddNewtonsoftJson` 新しい MVC サービスの登録メソッドと互換性があります。
...
3\.0 では、ランタイムのコンパイルは、オプトイン シナリオです。 ランタイムのコンパイルを有効にするのを参照してください。 https://docs.microsoft.com/aspnet/core/mvc/views/view-compilation?view=aspnetcore-3.0#runtime-compilation します。

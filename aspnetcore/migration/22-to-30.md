...
* アプリケーションで[API アナライザー](xref:web-api/advanced/analyzers)を使用する場合は`<PackageReference>` 、 [AspNetCore](https://www.nuget.org/packages/Microsoft.AspNetCore.Mvc.Api.Analyzers/)パッケージの要素をすべて削除します。 プロジェクトファイルを編集して、.NET Core SDK の一部として出荷されたアナライザーを使用します。
...
AspNetCore の 2.2 `AddAuthorization`メソッドとメソッドを次に示します。 ASP.NET Core

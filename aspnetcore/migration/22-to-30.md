...
* `AuthorizePage`アプリがや`[Authorize]`などの認証/承認機能を使用する場合は、の`UseAuthentication`呼び出し`UseAuthorization`をとの**後** `UseRouting`に配置します。
* アプリで[CORS](xref:security/cors)機能 (など) `[EnableCors]`を使用する場合は、[次へ] を配置`UseCors`します。

...
* テストプロジェクトは、テストを格納して実行するために使用されます。 テストプロジェクトには、テスト*対象のシステム*(SUT) と呼ばれるテストされた ASP.NET Core プロジェクトへの参照が含まれています。 _"SUT" は、このトピック全体で、テスト対象のアプリを参照するために使用されます。_
* テストプロジェクトでは、SUT のテスト web ホストを作成し、テストサーバークライアントを使用して、SUT に対する要求と応答を処理します。
...
[Webapplicationfactory&lt;の&gt;](/dotnet/api/microsoft.aspnetcore.mvc.testing.webapplicationfactory-1)このサーバーを使用して、統合テスト用の[testserver](/dotnet/api/microsoft.aspnetcore.testhost.testserver)を作成します。 `TEntryPoint`は、SUT のエントリポイントクラス (通常`Startup`はクラス) です。
...
`IndexPageTests`クラス内の別のテストによって、データベース内のすべてのレコードを削除し、メソッドの`Post_DeleteMessageHandler_ReturnsRedirectToRoot`前に実行できる操作が実行されるため、このテストメソッドでは、SUT が削除するレコードが存在することを確認するためにデータベースがシード処理されます。 Sut で`messages`フォームのボタンを選択すると、sutに対する要求でシミュレートされます。`deleteBtn1`
